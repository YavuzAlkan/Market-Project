function loadProducts() {
    if (!window.location.hash) {
      ...
      fetch("http://localhost:5242/api/Products/GetProducts")
    ...
    function loadCategoriesIntoSelect() {
        fetch("http://localhost:5242/api/Categories/GetCategories")
            .then((response) => response.json())
            .then((categories) => {
    ...
    function createProduct() { 
        ...
        if (form) {
            form.addEventListener("submit", (event) => {
                ...
                fetch("http://localhost:5242/api/Products/CreateProduct", {
                    method: "POST",
                    body: data,
     ...
     function updateProductGet(){
      ...
      fetch("http://localhost:5242/api/Products/GetProductById?id=" + productId)
      .then((response) => response.json())
      .then((data) =>{
       ...
       function updateProductPost(event){
        ...
        const data=new FormData(form);
         fetch("http://localhost:5242/api/Products/UpdateProduct",{
          method:"PUT",
          body:data
         }).then((response)=>console.log(response))
         .then(()=>{
       ...
       function deleteProduct(id){
        ...
        if(result){
          fetch("http://localhost:5242/api/Products/DeleteProduct?id=" +id,{
            method:"DELETE"
          })
          .then(()=>{
            loadProducts()
