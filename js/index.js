...
fetch("http://localhost:5242/api/Categories/GetCategoryById?id=" + categoryId)
.then((response) => response.json())
.then((data) =>{
 ...
 function updateCategoryPost(event){
  ...
  const data=new FormData(form);
   fetch("http://localhost:5242/api/Categories/UpdateCategory",{
    method:"PUT",
    body:data
   }).then((response)=>console.log(response))
   .then(()=>{
 ...
 function deleteCategory(id){
  ...
  if(result){
    fetch("http://localhost:5242/api/Categories/DeleteCategory?id=" +id,{
      method:"DELETE"
    })
    .then(()=>{
      loadCategories()
